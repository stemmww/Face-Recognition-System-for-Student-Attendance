...
A blink challenge would need landmarks around each eye (classic EAR uses
six per eye); SCRFD's single point per eye is not enough, so blink is
omitted from this pipeline.
...
def validate_challenge(
    ...
    if challenge == ChallengeType.TURN_RIGHT:
        ...
        return ok, "Head turn detected" if ok else "Head turn not detected — please try again"
    if challenge == ChallengeType.NOD:
